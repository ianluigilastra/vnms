Template.addNewProductInfo.helpers({
	NPIdata : function() {
	...
	brands: function () {
		return LuBrands.find({},{sort : {name : 1}}).fetch();
	},
...
Template.addNewProductInfo.rendered=function() {
...
Template.addNewProductInfo.events({
	'click button#btn-npi-save' : function(e,t){
		if($('#npi-form').parsley().validate()){
				...
				Meteor.call('insertProductInfo',newProdInfo, function(error, response){
		            if(error){
		                alert(error.reason);
		              }else{
		              	TempCol.remove({});
...
Template.addNewProductInfo.created = function() {
	var self = this;
	delete Session.keys['uploaded'];
	delete Session.keys['uploadedjpg'];
