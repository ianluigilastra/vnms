Template.addNewFastFacts.helpers({
	NPIdata : function() {
...
Template.addNewFastFacts.rendered=function() {
...
Template.addNewFastFacts.events({
	'click button#btn-npi-save' : function(e,t){
		if($('#vnm-form').parsley().validate()){	
			...
			Meteor.call('insertFastFact',fastFact, function(error, response){
	            if(error){
	                alert(error.reason);
	              }else{
	              	TempCol.remove({});
...
Template.addNewFastFacts.created = function() {
	var self = this;
